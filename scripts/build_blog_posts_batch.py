...
ARTICLES = [
    {
        ...
        'eyebrow': 'О проекте',
        'lead': 'Кто стоит за АБХАЗБЕРЕГ, как устроен каталог и четыре конкретные выгоды бронирования напрямую — без воды.',
...
{body_ps}
        ...
        <p class="blog-source">Источник: <a href="https://t.me/abhazbooking/{tid}" target="_blank" rel="noopener noreferrer">пост Telegram @abhazbooking/{tid}</a>.</p>
          </div>
        </div>
