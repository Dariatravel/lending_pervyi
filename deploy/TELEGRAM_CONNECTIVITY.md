...
## Автоперенос медиа из комментариев (полный автомат)

Для изменённых/новых объектов workflow после синка запускает
apply_telegram_supplemental_comments.py --from-watch-targets --force:
slug'и берутся из отчёта watch, комментарии под постом сканируются напрямую
(альбомы/одиночные фото и видео с осмысленной подписью; отзывы отфильтровываются),
медиа скачиваются и заливаются в бакет, секция вставляется в страницу и
сохраняется в data/supplemental-blocks.json — поэтому переживает пересборки.
Аудит-файл с Mac больше не обязателен (остаётся для ручных массовых прогонов).
