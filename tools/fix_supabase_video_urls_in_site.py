#!/usr/bin/env python3
"""Replace Supabase Storage video URLs with Yandex CDN in site files."""
...
SUPABASE_VIDEO_RE = re.compile(
    r"https://chnyazvybzzryduhgopa\.supabase\.co/storage/v1/object/public/site-media/videos/[^\s\"'<>]+",
    re.I,
)
...
def rewrite_text(text: str) -> tuple[str, int]:
    ...
    def repl(match: re.Match[str]) -> str:
        nonlocal count
        old = match.group(0)
        new = yandex_video_url(old.split("?", 1)[0])
        if new != old:
            count += 1
        return new

    return SUPABASE_VIDEO_RE.sub(repl, text), count
