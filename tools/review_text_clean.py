...
_PREFIX_PATTERNS = [
    ...
    # «Евгения бо Отдали июне…» (Грант, 25.09.2026). Обрывок — 1–2 строчные
    # буквы и не настоящее короткое слово, следом — слово с заглавной.
    # Три буквы не берём: «Гостевой дом Самшит…» — «дом» настоящее слово,
    # пробный прогон 25.09 срезал бы его зря.
    re.compile(rf'^\s*[А-ЯЁ][а-яё]+\s+(?!{_SHORT_REAL_WORDS}\s)[а-яё]{{1,2}}\s+(?=[А-ЯЁ][а-яё])'),
