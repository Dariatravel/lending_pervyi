...
NOISE_RX = re.compile(
    ...
    r"условия\s+бронирования|правила|бронирования?)\s*$|^\s*\d+\s*$"
    # Подписи самой площадки, а не условия объекта.
    r"|как\s+бронировать|сервис\s+бронирован|товарный\s+знак|акци\w+\s+«|скидка\s+на\s+перв"
    r"|официальный\s+сайт",
    re.I,
)

# Отзывы гостей («забронировали, внесли предоплату, вернули только половину»)
# — это не правило отеля, но и не мусор: такие истории показывают, как условия
# работают на деле. Помечаем отдельно, чтобы не путать с опубликованными правилами.
REVIEW_RX = re.compile(
    r"\b(?:забронировал|внесл|отменил|вернул|оплатил|приехал|отдыхал|нам\s+|мы\s+)\w*", re.I
)
...
def expand(page) -> int:
    ...
    clicks = 0
    # Часть площадок (alean.ru) прячет правила отмены под своим заголовком —
    # «Изменнение или аннуляция», «Оплата». Без клика по нему виден только заголовок.
    labels = ("Условия", "Правила", "Показать", "Подробнее", "Ещё", "Еще", "Развернуть",
              "Аннуляц", "Отмена", "Изменнение", "Изменение", "Оплата", "Бронирование")
    for label in labels:
        try:
...
def harvest(page) -> list[dict]:
    ...
    for chunk in chunks:
        ...
        found.append({
            "text": chunk,
            "topic": ("ОТЗЫВ ГОСТЯ" if REVIEW_RX.search(chunk) else
                      "отмена" if CANCEL_RX.search(chunk) else
