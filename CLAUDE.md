...
- Lighthouse: отель LCP ~7.4s по последнему замеру — прирост от preload не подтверждён,
  нужно перемерить прод и при необходимости найти реальный LCP-элемент.
...
- K4: concept-*.html и код ботов (collab_bot, cashback_tracker) всё ещё публикуются
  Pages (закрыты robots.txt) — решение о выносе за владелицей.
